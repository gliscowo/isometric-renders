...
import static com.glisco.isometricrenders.Translator.trString;
...
public abstract class RenderScreen extends Screen {
    ...
    protected void init() {
        ...
        CheckboxWidget playAnimationsCheckbox = new CallbackCheckboxWidget(viewportEndX + 10, 68, tr("gui.isometric-renders.animations"), playAnimations, aBoolean -> {
            playAnimations = aBoolean;
        });
        CheckboxWidget playParticlesCheckbox = new CallbackCheckboxWidget(viewportEndX + 10, 93, tr("gui.isometric-renders.particles"), tickParticles, aBoolean -> {
            tickParticles = aBoolean;
        });

        CheckboxWidget doHiResCheckbox = new CallbackCheckboxWidget(viewportEndX + 10, 183, tr("gui.isometric-renders.use_external_renderer"), useExternalRenderer, aBoolean -> {
            useExternalRenderer = aBoolean;
        });
        CheckboxWidget allowMultipleRendersCheckbox = new CallbackCheckboxWidget(viewportEndX + 10, 208, tr("gui.isometric-renders.allow_multiple_export_jobs"), allowMultipleNonThreadedJobs, aBoolean -> {
            allowMultipleNonThreadedJobs = aBoolean;
        });
        CheckboxWidget dumpIntoRootCheckbox = new CallbackCheckboxWidget(viewportEndX + 10, 233, tr("gui.isometric-renders.dump_into_root"), dumpIntoRoot, aBoolean -> {
            dumpIntoRoot = aBoolean;
        });

        ButtonWidget clearQueueButton = new ButtonWidget(viewportEndX + 80, 260, 75, 20, tr("gui.isometric-renders.clear_queue"), button -> {
            ImageExporter.clearQueue();
        });
        exportButton = new ButtonWidget(viewportEndX + 10, 260, 65, 20, tr("gui.isometric-renders.export"), button -> {
        ...
        ButtonWidget rendersFolderButton = new ButtonWidget(viewportEndX + 10, 285, 90, 20, tr("gui.isometric-renders.open_folder"), button -> {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (!drawOnlyBackground) {
            ...
            client.textRenderer.draw(matrices, tr("gui.isometric-renders.render_options"), viewportEndX + 12, 20, 0xAAAAAA);
            client.textRenderer.draw(matrices, tr("gui.isometric-renders.background_color"), viewportEndX + 66, 43, 0xFFFFFF);
            fill(matrices, viewportEndX + 160, 43, viewportEndX + 168, 51, RuntimeConfig.backgroundColor | 255 << 24);

            client.textRenderer.draw(matrices, tr("gui.isometric-renders.export_options"), viewportEndX + 12, 135, 0xAAAAAA);
            client.textRenderer.draw(matrices, tr("gui.isometric-renders.renderer_resolution"), viewportEndX + 66, 160, 0xFFFFFF);

            client.textRenderer.draw(matrices, tr("gui.isometric-renders.hotkeys"), viewportEndX + 12, height - 20, 0xAAAAAA);

            int y = height - 60;
            for (String line : trString("gui.isometric-renders.memory_warning").split("\\n")) {
                client.textRenderer.draw(matrices, line, 10, y, 0xAAAAAA);
                y -= 10;
            }
